...
export const DeploymentConfigurationWizard: React.FC<DeploymentConfigurationWizardProps> = ({
  ...
  const isConfigComplete = (): boolean => {
    return !!(config.deployment_id && config.name && config.platform && config.model_path);
  };
        ...
        <div className="flex items-center justify-between">
          {["platform", "model", "configuration", "review"].map((step, index) => (
            <React.Fragment key={step}>
              ...
              {index < 3 && <div className="flex-1 h-1 bg-gray-200 mx-2" />}
            </React.Fragment>
          ))}
        </div>
      </div>
          ...
          {currentStep === "review" ? (
            ...
            <button
              ...
              disabled={
                ...
                (currentStep === "model" &&
                  (!config.name || !config.deployment_id || !config.model_path))
              }
              className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Next
