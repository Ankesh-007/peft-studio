...
const PausedRunExample: React.FC<PausedRunExampleProps> = ({ jobId }) => {
  ...
  const fetchPausedRunInfo = async (id: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/training/paused/${id}`,
      );
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch paused run",
      );
  ...
  const fetchPausedRuns = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch paused runs",
      );
  ...
  const handleResume = async (jobId: string) => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/training/resume/${jobId}`,
        {
          method: "POST",
        },
      );
      ...
      alert(
        `Failed to resume training: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
  ...
  const handleStop = async (jobId: string) => {
    if (
      !confirm(
        "Are you sure you want to stop this training run? Progress will be saved.",
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/training/stop/${jobId}`,
        {
          method: "POST",
        },
      );
      ...
      alert(
        `Failed to stop training: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] to-[#0f0f0f] p-24 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-body text-dark-text-secondary">
            Loading paused run information...
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] to-[#0f0f0f] p-24 flex items-center justify-center">
        <div className="card max-w-[600px]">
          <div className="text-center">
            ...
            <button
              onClick={() =>
                jobId ? fetchPausedRunInfo(jobId) : fetchPausedRuns()
              }
