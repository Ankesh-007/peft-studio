...
const ModelBrowser: React.FC<ModelBrowserProps> = ({ onModelSelect }) => {
  ...
  const loadPopularModels = async () => {
    ...
    setError(null);
    try {
      const response = (await apiClient.getPopularModels("text-generation", 20)) as {
  ...
  const handleSearch = useCallback(async (searchFilters: ModelSearchFilters) => {
    ...
    try {
      ...
      const registries =
        searchFilters.registries && searchFilters.registries.length > 0
  ...
  const handleAddToComparison = (model: ModelMetadata) => {
    if (
      ...
      !comparisonModels.find((m) => m.model_id === model.model_id)
    ) {
  ...
  const loadCachedModels = async () => {
    try {
      const response = (await apiClient.getCachedModels()) as {
        cached_models: Array<{ id: string; name: string;[key: string]: unknown }>;
      };
  ...
  const handleRemoveFromCache = async (modelId: string, registry: string) => {
    try {
      ...
      setCachedModels(
        cachedModels.filter((c) => !(c.model_id === modelId && c.registry === registry))
      );
    } catch (err) {
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Model Browser</h1>
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
              ...
              <button
                ...
                className={`px-3 py-1 rounded ${view === "list"
                  ? "bg-white dark:bg-gray-600 shadow"
                  : "text-gray-600 dark:text-gray-400"
                  }`}
              >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 overflow-y-auto">
          <ModelFilters filters={filters} onFiltersChange={handleSearch} />
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-6 flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Cached Models</h2>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <h3 className="font-semibold text-gray-900 dark:text-white">
                            {(cached.metadata as any)?.model_name || cached.name}
                          </h3>
                          <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded text-xs">
                            {cached.registry as React.ReactNode}
                          </span>
                        </div>
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                          by {(cached.metadata as any)?.author || "Unknown"}
                        </p>
                        <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                          <span>Cached: {new Date(cached.cached_at as string).toLocaleString()}</span>
                          <span>Expires: {new Date(cached.expires_at as string).toLocaleString()}</span>
                        </div>
                      </div>
                      <button
                        onClick={() => handleRemoveFromCache(cached.model_id as string, cached.registry as string)}
