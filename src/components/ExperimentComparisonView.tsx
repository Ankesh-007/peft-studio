...
export const ExperimentComparisonView: React.FC<ExperimentComparisonViewProps> = ({
  ...
  const getBestPerformer = (metricName: string): string | null => {
    ...
    const isLossMetric = metricName.toLowerCase().includes('loss') || 
                  ...
                  return (
                    <tr key={metricName}>
                      ...
                      {comparisonData.experiments.map((exp) => {
                        ...
                        const diff = bestJobId ? getMetricDifference(exp.job_id, metricName, bestJobId) : 0;
                            ...
                            {value !== undefined ? (
                              <div>
                                <div className="flex items-center gap-2">
                                  <span className={isBest ? 'font-bold text-green-700' : 'text-gray-900'}>
                ...
                {Object.keys(
                  ...
                  <tr key={paramName}>
                    ...
                    {comparisonData.experiments.map((exp) => {
                      ...
                      return (
                        <td key={exp.job_id} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {value !== undefined ? (
                            typeof value === 'object' ? (
          ...
          {comparisonData.experiments.map((exp) => (
            <div key={exp.job_id} className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <div className="text-sm text-gray-600">Tracker</div>
                  <div className="text-base font-medium text-gray-900">{exp.tracker}</div>
                </div>
                
                <div>
                  <div className="text-sm text-gray-600">Model</div>
                  <div className="text-base font-medium text-gray-900">
                    {exp.metadata.model_name}
                  </div>
                </div>
