...
const ModelComparisonView: React.FC<ModelComparisonViewProps> = ({
  ...
  const comparisonMetrics = [
    ...
    { key: "architecture", label: "Architecture", format: (v: unknown) => (v ? String(v) : "N/A") },
    { key: "license", label: "License", format: (v: unknown) => (v ? String(v) : "N/A") },
    { key: "library_name", label: "Library", format: (v: unknown) => (v ? String(v) : "N/A") },
  ];
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {comparisonMetrics.map((metric) => (
                ...
                >
                  <td className="p-4 font-medium text-gray-900 dark:text-white">{metric.label}</td>
                  {models.map((model) => {
                    ...
                    return (
                      <td key={model.model_id} className="p-4 text-center">
                        <span
                          className={`inline-block px-3 py-1 rounded ${isBest
