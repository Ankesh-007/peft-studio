  ...
  ({ model, onClick, onAddToComparison, isInComparison }) => {
    return (
      ...
      >
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">by {model.author}</p>
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          {model.downloads && <span>↓ {formatNumber(model.downloads)}</span>}
          {model.likes && <span>♥ {formatNumber(model.likes)}</span>}
        </div>
...
export const OptimizedModelGrid: React.FC<OptimizedModelGridProps> = memo(
  ({ models, view, onModelClick, onAddToComparison, comparisonModels = [] }) => {
    ...
    const Cell = useCallback(
      ({
        ...
        const index = view === "grid" ? rowIndex * gridConfig.columnCount + columnIndex : rowIndex;
          ...
          <div style={style} className="p-2">
            {view === "grid" ? (
              <ModelCard
                ...
                onAddToComparison={
                  onAddToComparison ? () => handleAddToComparison(model) : undefined
                }
