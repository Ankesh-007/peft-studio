import { TooltipConfig } from '../types/wizard';
...
export const tooltipConfig: TooltipConfig = {
  ...
  'profile.use_case': {
    title: 'Use Case',
    description: 'The type of task you want your model to perform. Each use case has optimized settings for that specific purpose.',
  ...
  'lora_r': {
    title: 'LoRA Rank (r)',
    description: 'LoRA rank controls how many parameters are trained. Higher values mean more learning capacity but require more memory.',
    example: 'Typical values: 8 for simple tasks, 16-32 for complex tasks.'
  },
  'lora_alpha': {
    title: 'LoRA Alpha',
    description: 'LoRA alpha is a scaling factor that controls how much the fine-tuning affects the model. Usually set to 2x the rank.',
    example: 'If rank is 16, alpha is typically 32.'
  },
  'lora_dropout': {
    title: 'LoRA Dropout',
    description: 'LoRA dropout randomly turns off some training to prevent overfitting. Higher values (0.1-0.2) help with small datasets.',
    example: '0.05 for large datasets, 0.1-0.2 for small datasets.'
  },
  'target_modules': {
    title: 'Target Modules',
    description: 'LoRA target modules specify which parts of the model to train. More modules = better results but slower training.',
  ...
  'learning_rate': {
    title: 'Learning Rate',
    description: 'How quickly the model learns. Too high causes instability, too low makes training slow.',
    example: 'Typical range: 1e-4 to 3e-4 for fine-tuning.'
  },
  'num_epochs': {
    title: 'Number of Epochs',
    description: 'How many times the model sees your entire dataset. More epochs = more learning, but risk of overfitting.',
    example: '3-5 epochs is typical for most tasks.'
  },
  'warmup_ratio': {
    title: 'Warmup Ratio',
    description: 'Percentage of training where learning rate gradually increases. Helps stabilize early training.',
    example: '0.1 means 10% of training is warmup.'
  },
  'max_seq_length': {
    title: 'Maximum Sequence Length',
    description: 'Maximum number of tokens (words/pieces) the model can process at once. Longer = more context but more memory.',
    example: '2048 for chat, 4096 for code or long documents.'
  },
  'weight_decay': {
    title: 'Weight Decay',
    description: 'Regularization that prevents overfitting by penalizing large weights. Small values (0.01) are typical.',
    example: '0.01 is a good default for most tasks.'
  },
  'max_grad_norm': {
    title: 'Gradient Clipping',
    description: 'Prevents training instability by limiting how much the model can change in one step.',
    example: '1.0 is standard; lower (0.5) for unstable training.'
  },
  'scheduler': {
    title: 'Learning Rate Scheduler',
    description: 'How the learning rate changes during training. Cosine gradually decreases, linear drops steadily.',
  ...
  'batch_size': {
    title: 'Batch Size',
    description: 'Number of examples processed together. Larger batches are faster but use more memory.',
    example: 'Typical values: 4-8 for 8GB GPU, 16-32 for 24GB GPU.'
  },
  'gradient_accumulation': {
    title: 'Gradient Accumulation',
    description: 'Simulates larger batches by accumulating gradients over multiple steps. Useful when memory is limited.',
  ...
  'precision': {
    ...
    description: 'Number format used during training. Lower precision (fp16, bf16) uses less memory and is faster.',
    example: 'fp16 for most GPUs, bf16 for newer GPUs (A100, H100).'
  },
  'quantization': {
    title: 'Quantization',
    description: 'Compresses the model to use less memory. 8-bit is good quality, 4-bit saves more memory.',
  ...
  'dataset_format': {
    ...
    description: 'The file format of your training data. We support common formats like CSV, JSON, and plain text.',
    example: 'Typical formats: CSV (80% of users), JSON/JSONL (15%), TXT (5%).'
  },
  'dataset_validation': {
    title: 'Dataset Validation',
    description: 'Automatic checks to ensure your data is properly formatted and ready for training.',
  ...
  'model_selection': {
    ...
    description: 'Choose a pre-trained model to fine-tune. Consider model size, popularity, and compatibility with your hardware.',
    example: 'Popular choices: Llama-2-7B (70k+ downloads), Mistral-7B (50k+ downloads), Phi-2 (3B parameters).'
  },
  'model_size': {
    title: 'Model Size',
    description: 'Number of parameters in the model. Larger models are more capable but require more resources.',
  ...
  'model_architecture': {
    ...
    example: 'Common architectures: Llama-2 (60% of users), Mistral (25%), GPT-style (10%), others (5%).'
  },
  
  // Training Estimates
  'training_estimates': {
    title: 'Training Estimates',
    description: 'Predicted time, cost, and resource usage for your training run. These are estimates and may vary by ±20%.',
  ...
  'epochs': {
    ...
    description: 'Number of complete passes through your dataset. More epochs = more learning but risk of overfitting.',
  ...
  'optimizer': {
    title: 'Optimizer',
    description: 'Algorithm used to update model weights. AdamW is the standard for LLM fine-tuning.',
    example: 'Use AdamW (default) for 99% of cases, SGD only for specific research needs.'
  },
  'save_steps': {
    title: 'Save Checkpoint Every N Steps',
    description: 'How often to save your progress. More frequent saves use more disk space but provide more recovery points.',
