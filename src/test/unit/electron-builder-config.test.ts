...
describe("Electron Builder Configuration Properties", () => {
  ...
  test("Property 3: Correct file formats per platform", () => {
    fc.assert(
      fc.property(fc.constantFrom("windows", "macos", "linux"), (platform) => {
  ...
  test("Property 16: macOS DMG provided", () => {
    ...
    // Extract target names
    const targets = buildConfig.mac.target.map((t: any) => (typeof t === "string" ? t : t.target));
    ...
    const hasAppsLink = contents.some(
      (item: any) => item.type === "link" && item.path === "/Applications"
    );
