      ...
      fc.property(
        ...
        ([major, minor, patch]) => {
          ...
          const releaseNotes = template
            ...
            .replace(/\{\{RELEASE_NOTES\}\}/g, "Test changelog")
          ...
          expect(releaseNotes).toContain("### Windows");
          expect(releaseNotes).toContain("### macOS");
          expect(releaseNotes).toContain("### Linux");

          // Check that instructions contain actionable steps
          expect(releaseNotes).toMatch(/1\.\s+Download/);
          expect(releaseNotes).toMatch(/2\.\s+\w+/); // Step 2
          expect(releaseNotes).toMatch(/3\.\s+\w+/); // Step 3
    ...
    fc.assert(
      fc.property(
        ...
        ({ major, minor, patch, platform }) => {
          ...
          // Find the Installation Instructions section first
          const installHeader = "## Installation Instructions";
          const installIndex = releaseNotes.indexOf(installHeader);
          expect(installIndex).toBeGreaterThan(-1);

          const installSection = releaseNotes.substring(installIndex);

          // Find the platform header within the installation section
          const headerIndex = installSection.indexOf(checks.header);
          expect(headerIndex).toBeGreaterThan(-1);

          const afterHeader = installSection.substring(headerIndex);

          // Find end of this platform section
          // It ends at the next "### " or "## "
          let endIndex = afterHeader.length;
          const nextH2 = afterHeader.indexOf("\n## ", 1);
          const nextH3 = afterHeader.indexOf("\n### ", 1);

          if (nextH2 > 0 && nextH2 < endIndex) endIndex = nextH2;
          if (nextH3 > 0 && nextH3 < endIndex) endIndex = nextH3;

          const platformSection = afterHeader.substring(0, endIndex);
