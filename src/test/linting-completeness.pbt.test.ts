...
describe('Linting Completeness - Property-Based Tests', () => {
  ...
  describe('Property 1: Linting Completeness', () => {
    ...
    it('should have zero errors and warnings for React components', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.subarray(componentFiles, { minLength: 1, maxLength: Math.min(10, componentFiles.length) }),
          async (filesToLint) => {
            const results = await lintFiles(filesToLint);
            ...
            for (const result of results) {
              const reactViolations = result.messages.filter(msg => 
                msg.ruleId?.startsWith('react/') || 
                msg.ruleId?.startsWith('react-hooks/')
              );
