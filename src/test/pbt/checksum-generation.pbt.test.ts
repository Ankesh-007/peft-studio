  ...
  it('should generate SHA256 checksum for any installer file', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.constantFrom('.exe', '.dmg', '.zip', '.AppImage', '.deb').chain(ext =>
  ...
  it('should generate checksums for all installer files in a directory', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.array(
          fc.record({
            name: fc.constantFrom('.exe', '.dmg', '.zip', '.AppImage', '.deb').chain(ext =>
  ...
  it('should write all checksums to SHA256SUMS.txt in correct format', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(
          fc.record({
            name: fc.constantFrom('.exe', '.dmg', '.AppImage').chain(ext =>
