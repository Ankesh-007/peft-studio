...
describe("PEFT Algorithm Completeness Property Tests", () => {
  ...
  it("should have all expected algorithms available in the backend", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...EXPECTED_ALGORITHMS), (expectedAlgorithm: PEFTAlgorithm) => {
        // Check that the algorithm exists in the backend response
        const algorithmExists = backendAlgorithms.some((algo) => algo.id === expectedAlgorithm);
  ...
  it("should provide complete information for each algorithm", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...EXPECTED_ALGORITHMS), (algorithmId: PEFTAlgorithm) => {
        ...
        const hasRequiredFields =
          ...
          Array.isArray(algorithm.use_cases) &&
          algorithm.use_cases.length > 0 &&
          Array.isArray(algorithm.requirements) &&
          algorithm.requirements.length > 0 &&
          Array.isArray(algorithm.advantages) &&
          algorithm.advantages.length > 0 &&
          Array.isArray(algorithm.disadvantages) &&
          algorithm.disadvantages.length > 0 &&
          algorithm.memory_efficiency.length > 0 &&
          algorithm.training_speed.length > 0 &&
          Array.isArray(algorithm.parameters) &&
  ...
  it("should provide valid parameter definitions for each algorithm", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...EXPECTED_ALGORITHMS), (algorithmId: PEFTAlgorithm) => {
        ...
        return algorithm.parameters.every(
          (param) =>
            param.name.length > 0 &&
