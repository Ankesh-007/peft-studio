...
const LATEST_RELEASE_PATTERN =
  /https:\/\/github\.com\/[a-zA-Z0-9-]+\/peft-studio\/releases\/latest/;
...
describe("Download Links", () => {
  ...
  it("should have download links pointing to GitHub releases", () => {
    fc.assert(
      fc.property(fc.constantFrom(...DOCUMENTATION_FILES), (filePath) => {
        ...
        const peftDownloadPattern =
          /\[.*?(?:Download|download|Release|release|Installer|installer).*?PEFT.*?\]\((https?:\/\/[^)]+)\)/g;
        const peftReleasesPattern =
          /\[.*?(?:releases page|Releases|releases)\]\((https?:\/\/[^)]+)\)/g;
  ...
  it("should have prominent download link in README", () => {
    ...
    const allReleasesUrls =
      content.match(/https:\/\/github\.com\/[a-zA-Z0-9-]+\/peft-studio\/releases[^\s)"\]']*/g) ||
      [];
      ...
      fc.property(fc.constantFrom(...DOCUMENTATION_FILES), (filePath) => {
        ...
        matches.forEach((url) => {
          ...
          const baseUrl = url.match(
            /https:\/\/github\.com\/[a-zA-Z0-9-]+\/peft-studio\/releases/
          )?.[0];
          if (baseUrl) {
  ...
  it("should have download instructions in installation guides", () => {
    ...
    fc.assert(
      fc.property(fc.constantFrom(...installationGuides), (filePath) => {
        const fullPath = path.join(process.cwd(), filePath);
