  ...
  it('should provide actionable fix instructions', () => {
    fc.assert(
      fc.property(errorInfoArb, (errorInfo) => {
        ...
        const actionWords = [
          'install', 'download', 'check', 'verify', 'run', 'restart',
          'close', 'open', 'click', 'try', 'ensure', 'make sure',
          'update', 'configure', 'set', 'add', 'remove'
        ];
        
        // At least one instruction should contain an action word
        const hasActionableInstruction = errorInfo.fixInstructions.some(
          (instruction) => {
    ...
    it('missing_packages errors should mention packages or dependencies', () => {
      ...
      fc.assert(
        fc.property(packagesErrorArb, (errorInfo) => {
          ...
          const mentionsPackages = allText.includes('package') || 
