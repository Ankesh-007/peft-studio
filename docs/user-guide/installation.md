# Installation Guide

Choose your platform and follow the installation steps below.

## Windows

1. **Download** PEFT-Studio-Setup.exe from [releases](https://github.com/yourusername/peft-studio/releases/latest)
2. **Run the installer**
3. **Follow the installation wizard**
4. **Launch PEFT Studio** from the Start Menu

### System Requirements
- Windows 10 or later
- 4GB RAM minimum
- 2GB disk space

---

## macOS

1. **Download** PEFT-Studio.dmg from [releases](https://github.com/yourusername/peft-studio/releases/latest)
2. **Open the DMG file**
3. **Drag PEFT Studio to Applications**
4. **Launch** from Applications folder

### System Requirements
- macOS 10.15 or later
- 4GB RAM minimum
- 2GB disk space

---

## Linux

1. **Download** PEFT-Studio.AppImage from [releases](https://github.com/yourusername/peft-studio/releases/latest)
2. **Make it executable:**
   ```bash
   chmod +x PEFT-Studio.AppImage
   ```
3. **Run:**
   ```bash
   ./PEFT-Studio.AppImage
   ```

### System Requirements
- Ubuntu 20.04+ or equivalent
- 4GB RAM minimum
- 2GB disk space

---

## Troubleshooting

### Windows
- If Windows Defender blocks the installer, click "More info" and then "Run anyway"
- Make sure you have administrator privileges

### macOS
- If you see "App can't be opened because it is from an unidentified developer":
  - Right-click the app and select "Open"
  - Click "Open" in the dialog

### Linux
- If the AppImage doesn't run, install FUSE:
  ```bash
  sudo apt install libfuse2
  ```

## Next Steps

After installation, check out the [Quick Start Guide](quick-start.md) to get started with PEFT Studio.
